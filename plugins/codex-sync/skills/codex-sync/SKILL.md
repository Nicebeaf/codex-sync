---
name: codex-sync
description: Safely share user-authored Codex skills, rules, AGENTS.md, and optional memories between Macs through the user's own private shared folder, with reviewed plans, store identity, handoff receipts, snapshots, and conflict preservation. Never copy credentials, sessions, logs, caches, or databases.
---

# Codex Sync

Use `scripts/codex_sync.py` for every operation. Version 0.2 binds writes to a reviewed plan, identifies each store and device, verifies cross-Mac handoffs with receipts, keeps snapshot history, and retains the existing three-way comparison, lock, backups, conflict refusal, and no deletion propagation.
...
```bash
python3 scripts/codex_sync.py create --store "/absolute/private/CodexSync" --device "mac-mini"
```
...
- Allowed by default: `~/.agents/skills`, `~/.codex/skills` except `.system`, `~/.codex/rules`, and `~/.codex/AGENTS.md`.
